...
flowchart TD
    ...
    GW --> CONF["wing.conf.ps1\n(local overrides)"]
    CONF -->|paths + CMDRs| GW
    SB --> BOX1["Sandbox: CMDR 1\nMinEdLauncher → ED"]
    SB --> BOX2["Sandbox: CMDR 2\nMinEdLauncher → ED"]
    SB --> BOXN["Sandbox: CMDR N\n..."]
    GW -->|after launch| WP["Win32 Window\nPositioning"]
    GW -->|skipIntro=true| CS["cutscene.ps1\nDismiss intro"]
    GW -->|pgEntry=true| PG["continue-pg.ps1\nPrivate Group entry"]
    BOX1 -.->|FSD jump event| AH["autohonk.py\n(per sandbox)"]
    BOX2 -.->|FSD jump event| AH
    BOXN -.->|FSD jump event| AH
    AH -->|holds Primary Fire| AH2["until FSSDiscoveryScan\nor timeout"]
```
